...
export default () => {
  ...
  const initSkillCooldown = (useSkill) => {
    const { status } = gameStatus.player

    skillCooldown = useSkill.cooldown

    // icon本體
    skillIcon = useSkill.icon;
    ...
    cooldown.roundRect(0, 0, size, size, arcRadius);
    cooldown.fill({ color: 'black', alpha: 0.7 })
    cooldown.width = size;
    cooldown.height = size;
    // 冷卻遮罩
    ...
    skillContainer.addChild(cooldown);
    ...
    let phase = 0;
    const animate = () => {
      const { lastSkillTime } = status
      const { gameTimer } = gameStatus
      const elapsed = gameTimer - lastSkillTime; // 經過秒數
      const remaining = Math.max(0, skillCooldown - elapsed); // 剩餘秒數

      // 印出剩餘時間
      timerText.text = Math.floor(remaining) + 1

      // 每秒前進 2π，因此每幀前進量是：
      phase = (elapsed / skillCooldown) * Math.PI * 2;
